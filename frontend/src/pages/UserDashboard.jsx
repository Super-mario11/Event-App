...
import {
  ...
  Share2,
  Star,
  IndianRupee, // Included for the 'Complete Payment' button
} from "lucide-react";
...
const generateQRCodeDataURL = async (text) => {
    try {
        // Dynamic import ensures the code only runs in the browser where the package is used
        const qrcodeModule = await import('qrcode'); 
        
        // Generate the QR code as a base64 Data URI string (CORS-safe)
        ...
        // If 'npm install qrcode' was not run, this catches the error and provides a fallback.
        console.error("Critical: Client-side QR generation failed. Please ensure 'npm install qrcode' was run.", e);
        // Fallback: Tiny transparent pixel (CORS-safe)
...
const UserDashboard = () => {
    ...
    try {
      ...
      const printContent = `
        ...
        <body>
          ...
          <script>
            // Data URI is instantly available; a minimal delay ensures final rendering and CSS are ready.
  ...
  const handleAddToCalendar = (eventTitle, eventDate, eventTime, venue) => {
    ...
    const endTime = eventTime ? new Date(startDateTime.getTime() + 60 * 60 * 1000) : startDateTime; 
    const formatCalendarTime = (date) => format(date, 'yyyyMMddTHHmmss');

    const googleCalendarUrl = `https://www.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(eventTitle)}&dates=${
      formatCalendarTime(startDateTime)
    }/${
      formatCalendarTime(endTime)
    }&details=${encodeURIComponent('Your confirmed booking with EventHive.')}&location=${encodeURIComponent(venue)}&sf=true&output=xml`;
  ...
  const upcomingCount = myBookings.filter(b => (b.status === 'confirmed' || b.status === 'created') && new Date(b.eventDate) >= new Date()).length; 
  const pastCount = myBookings.filter(b => b.status === 'confirmed' && new Date(b.eventDate) < new Date()).length;
              ...
              <div className="space-y-6">
                {filteredBookings.map((ticket) => {
                  ...
                  <div
                    ...
                    // Apply pending styling only if it's the upcoming tab
                    className={`border border-gray-200 rounded-xl p-6 hover:shadow-md transition-shadow 
                      ${isPending && activeTab === 'upcoming' ? 'bg-yellow-50 border-yellow-200 opacity-90' : ''} 
                      ${activeTab === 'past' ? 'opacity-70' : ''} 
                      ${activeTab === 'cancelled' ? 'bg-red-50 border-red-200 opacity-90' : ''}`}
                  >
                      ...
                      <div className="flex-1 space-y-3">
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
                          {ticket?.tickets &&
                            ticket.tickets.map((tkt, index) => (
                              <div
                                key={index}
                                className=" flex flex-col md:grid grid-cols-2 gap-4 text-sm"
                              >
                        ...
                        <div className="flex flex-wrap gap-3">
                          ...
                          {activeTab === 'upcoming' && (
                            <>
                              <button
                                onClick={() => handleDownloadTicket(ticket?.bookingId)}
                              ...
                              <button 
                                onClick={() => handleShareTicket(ticket?.eventTitle, ticket?.bookingId)}
            ...
            {activeTab === "past" && (
              <div className="space-y-6">
                {filteredBookings.map((ticket) => (
                  <div
                    key={ticket?.bookingId}
                    className="border border-gray-200 rounded-xl p-6 opacity-70"
                  >
                     <div className="flex flex-col lg:flex-row gap-6">
                      <img
                        src={ticket?.image}
                      ...
                      <div className="flex-1 space-y-3">
                        <div>
                          <h3 className="text-xl font-semibold text-red-700">
                            {ticket?.eventTitle} (Cancelled)
                          </h3>
                          <p className="text-sm text-red-600">Booking ID: {ticket.bookingId} - Cancelled on {format(new Date(ticket.updatedAt), 'MMM dd, yyyy')}</p>
                          <div className="flex items-center space-x-4 text-gray-600 mt-2">
                            <div className="flex items-center space-x-1">
                              <Calendar className="w-4 h-4" />
                              <span>{ticket?.eventDate?.slice(0, 10)}</span>
